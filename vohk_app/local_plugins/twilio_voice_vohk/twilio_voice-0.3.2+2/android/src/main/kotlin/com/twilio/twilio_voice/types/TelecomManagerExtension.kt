...
object TelecomManagerExtension {
    ...
    fun TelecomManager.registerPhoneAccount(ctx: Context, phoneAccountHandle: PhoneAccountHandle) {
        val existingPhoneAccount = getPhoneAccount(phoneAccountHandle)
        if (existingPhoneAccount?.hasCapabilities(PhoneAccount.CAPABILITY_SELF_MANAGED) == true) {
            Log.d("TelecomManager","registerPhoneAccount: self-managed phone account already registered.")
            return
        }
